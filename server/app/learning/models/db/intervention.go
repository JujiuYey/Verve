...
type LearningTeachingIntervention struct {
	...
	Evidence           []LearningTeachingEvidence `bun:"evidence,type:jsonb,notnull" json:"evidence"`                             // 文档与检索依据
	CreatedAt          time.Time                  `bun:"created_at,nullzero,notnull,default:current_timestamp" json:"created_at"` // 创建时间
}

// LearningTeachingEvidence 教学回答引用的文档证据
type LearningTeachingEvidence struct {
	ChunkID         string `json:"chunk_id"`         // 分块ID
	DocumentVersion int64  `json:"document_version"` // 文档版本号
