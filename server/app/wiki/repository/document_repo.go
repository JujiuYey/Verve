...
// 更新文档状态
func (r *DocumentRepository) UpdateStatus(ctx context.Context, docID string, status string, chunkCount int, errorMsg *string) error {
	doc := &wiki_db.Document{
		Status:       status,
	...
	_, err := r.db.NewUpdate().
		Model(doc).
		Column("status", "chunk_count", "error_message").
		Where("id = ?", docID).
		Exec(ctx)
