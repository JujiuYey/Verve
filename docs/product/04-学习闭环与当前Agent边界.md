...
| `LearningCoach` | 根据 Wiki 资料、学习小节、画像和学习记录决定下一步 | 独立 `ChatModelAgent`，带 Coach tools，输出可选 `<ACTION>` |
...
`LearningCoach` 位于这个闭环外侧，负责决定用户下一步应该进入哪个小节或是否需要先生成学习小节。
