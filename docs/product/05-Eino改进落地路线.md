...
学习风格 preset 是 prompt 结构化的合理动机。它说明为什么 prompt 不应该继续散落在 `const string` 里：后续同一个 Tutor / Examiner 可能要根据学习风格生成略有差异的指令。
...
- 需要证明用户真的会主动选择；
- 需要处理切换风格后的历史会话语义；
- 需要评估“风格”到底影响 Tutor、Examiner 还是整个 Orchestrator。
...
- 现有 ObjectiveGenerator / Coach / Tutor / Examiner 行为保持一致；
...
- ObjectiveGenerator 当前没有 tools 参数；
...
- `agent.go` 继续创建原来的 4 个 `ChatModelAgent`；
...
- 已经能证明两个 preset 在 Tutor 或 Examiner 上有清晰行为差异；
