...
allowed-tools: Bash(npx shadcn@latest *), Bash(npx --yes shadcn@latest *), Bash(pnpm dlx shadcn@latest *), Bash(bunx --bun shadcn@latest *)
---
...
## Verve Frontend Scope

This skill is installed project-locally for this repository only. The shadcn/ui frontend root is `web/`, and `web/components.json` is the source of truth.

- Run project-aware shadcn commands from the repository root with `--cwd web`, or `cd web` first.
- Use `npx --yes shadcn@latest ... --cwd web` when JSON output or latest CLI behavior is needed.
- Do not install this skill globally or run `skills add -g` for this project.

## Current Project Context

```json
!`npx --yes shadcn@latest info --json --cwd web`
```
...
1. **Get project context** — already injected above. In this repository, run `npx --yes shadcn@latest info --json --cwd web` again if you need to refresh.
